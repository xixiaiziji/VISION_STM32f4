...
uint16_t m[480][800]  __attribute__((at((uint32_t)0xD0300000)));
int16_t  o[480][800]  __attribute__((at((uint32_t)0xD0400000)));
uint16_t counters[933][180] __attribute__((at((uint32_t)0xD0600000))); 
uint16_t Hough[4][9] = {
{-1, 0, 1,
-1, 0, 1,
-1, 0, 1},
		     ...
		     if(imgn[i][j]==label)
				 {
				  ...
				  jp=j+1;
					if(im<0) im=0;
					if(ip>img_height-2) ip=img_height-2-1;
          if(jm<0) im=0;
          if(jp>img_width-2) ip=img_width-2-1;
...
for(k=0; k<cnt; k++)
	{
	size=0;
	for(i=20; i<img_height-20; i++)
	 {
		 for(j=200; j<img_width-200; j++)
		 {
		   if(imgn[i][j]==k)
			 {
				size++;
...
void point(uint16_t a[4])
{
	...
	for(i=20;i<img_height-20;i++)		//获得图像
	{
		for(j=200;j<img_width-200;j++)
		{
...
for (a=2;a<=img_height-2;a++)   
{
 ...
 for(i=1;i<=img_height;i++)
	  {
		OV2640_DMAvtolcd_Config((uint32_t)m[i],FSMC_LCD_ADDRESS+i*(img_width)*2,img_width);
		 Delay_ms(1);
	  }
 for(x=1;x<img_height-1;x++)
	 {
...
**********************************/		
uint16_t RGB565_to_Gray(uint16_t RGB565)
{
		uint16_t t;	
		uint16_t R,G,B;	
		R=RGB565&0xf800>>11;
...
return: uint8_t GRAY
**********************************/		
uint8_t GET8Gray(uint16_t RGB565)
{  
		uint8_t R,G,B;
		uint16_t Gray;	
		R=GETR_FROM_RGB16(RGB565);
		G=GETG_FROM_RGB16(RGB565);
		B=GETB_FROM_RGB16(RGB565);
...
**********************************/
uint32_t rgb565_2_rgb888(uint16_t RGB565)  
{   
	  uint8_t R2R1R0,G1G0,B2B1B0,rgb888_R,rgb888_G,rgb888_B;
		uint32_t RGB888;
		//RGB888补位运算
	  R2R1R0=(uint8_t)((RGB565&0x3800)>>11);
    G1G0=(uint8_t)((RGB565&0x0060)>>5);
    B2B1B0=(uint8_t)(RGB565&0007);
		//RGB888各分量计算
	  rgb888_R=GETR_FROM_RGB16(RGB565)|R2R1R0;     
		rgb888_G=GETG_FROM_RGB16(RGB565)|G1G0;  
		rgb888_B=GETB_FROM_RGB16(RGB565)|B2B1B0;  
...
{   
		...
		RGB565=(rgb565_R<<11)|(rgb565_G<<5)|rgb565_B;  
