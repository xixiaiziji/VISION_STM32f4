...
// 24 MHz input clock
const unsigned char ov2640_UXGA_YUV[][2]=
{
0xff, 0x01,
0x12, 0x80,
0xff, 0x00,
...
//uint16_t image[img_buf_size]={0};
//uint16_t image2[240][400];
...
void OV2640_DCMI_Init(void) 
{
  ...
  DCMI_Init(&DCMI_InitStructure); 	
	...
	//FSMC_LCD_ADDRESS为lcd底层首地址
 	 	 
	// OV2640_DMAtov_Config((uint32_t)image,(uint16_t)img_buf_size);
}	
...
/**
  * @brief  配置 DMA 以捕获摄像头数据直接到lcd屏
	* @param  源地址&DCMI—>DR;目的地址image；字->半字
...
void OV2640_DMAtolcd_Config(uint32_t DMA_Memory0BaseAddr,uint16_t DMA_BufferSize)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
  DMA_ITConfig(DMA2_Stream1,DMA_IT_TC,ENABLE); //DMA2_Stream1传输完成中断使能
}

/**
  * @brief  配置 DCMI/DMA实现DMA传数据到数组变量 
	* @param 源地址显存地址;目的地址image半字->半字
  * @param  DMA_BufferSize：本次传输的数据量(单位为字,即4字节)
  */
void OV2640_DMAvtolcd_Config(uint32_t v_Addr,uint32_t lcd_Addr,uint16_t DMA_BufferSize)
{
  ...
  DMA_InitStructure.DMA_Channel = DMA_Channel_1;  
  DMA_InitStructure.DMA_PeripheralBaseAddr =v_Addr;	//显存地址
  DMA_InitStructure.DMA_Memory0BaseAddr = lcd_Addr; //DMA传输的目的地址(数组变量地址)
  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToMemory;
  DMA_InitStructure.DMA_BufferSize =DMA_BufferSize; 						//传输的数据大小(传入的参数)
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Enable;
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;			//存储器地址自增
  DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_HalfWord;//显存单位为16
  DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord;//数组变量单位16
  DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;								//一般模式
  DMA_InitStructure.DMA_Priority = DMA_Priority_High;
  DMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Disable;
  DMA_InitStructure.DMA_FIFOThreshold = DMA_FIFOThreshold_Full;//阈值为4个字
  ...
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
  DMA_ITConfig(DMA2_Stream0,DMA_IT_TC,ENABLE); //DMA2_Stream0传输完成中断使能
}

/**
  * @brief  配置 DMA 以将image[]数据传给lcd
...
void OV2640_DMAlcdtov_Config(uint32_t lcd_Addr,uint32_t v_Addr,uint16_t DMA_BufferSize)
{
  ...
  DMA_InitStructure.DMA_Channel = DMA_Channel_1;  
  DMA_InitStructure.DMA_PeripheralBaseAddr =lcd_Addr;
  DMA_InitStructure.DMA_Memory0BaseAddr = v_Addr; //DMA传输的目的地址(传入的参数)	
  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToMemory;
  DMA_InitStructure.DMA_BufferSize = DMA_BufferSize;	/* 数据数目 */
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Enable;/* 数组地址使能自动递增功能 */
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable; /* lcd地址使能非自动递增功能 */
  DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_HalfWord; /* 源数据是半字大小(16位) */
  DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord; /* 目标数据也是半字大小(16位) */
...
//  OV2640_Window_Set(0,0,800,600);
	 ...
	 // OV2640_LightMode(0x00);
	 // OV2640_SpecialEffects(0x07);
...
void OV2640_UXGA_YUV422_Mode(void)//设置OV2640输出YUV422.（YUYV）格式（16位数据的高字节为亮度值Y）
{
	 uint32_t i;
   for(i=0; i<(sizeof(ov2640_UXGA_YUV)/2); i++)
  {
    OV2640_WriteReg(ov2640_UXGA_YUV[i][0], ov2640_UXGA_YUV[i][1]);
  }

  //OV2640_WriteReg(0xff, 0x01);//要设置0x15寄存器，必须让0xFF=0x01
  //OV2640_WriteReg(0x15, 0x00);//设置OV2640输出YUYV格式
	OV2640_OutSize_Set(img_width,img_height); 
  OV2640_LightMode(0x03);
	OV2640_SpecialEffects(0x07);
